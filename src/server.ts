...
// News subscribers for real-time updates
const newsSubscribers = new Set<(data: unknown) => void>();
...
async function vehicleFromMultipartForm(form: FormData): Promise<Vehicle> {
  ...
  // Handle lottery prize fields from admin form
  const name = formStringWithDefault(form, "name") || formStringWithDefault(form, "vehicle");
  const prizeTier = formString(form, "prizeTier") as "GRAND" | "MAJOR" | "STANDARD" | "BASIC" | "";
  ...
  return {
    ...
    totalTickets: totalTickets || undefined,
    drawUrl: drawUrl || undefined,
...
async function handleApiRequest(request: Request): Promise<Response | undefined> {
  ...
  if (url.pathname === "/api/news" && request.method === "POST") {
    let newsItem: Partial<LotteryNews>;
    try {
      newsItem = await readJsonBody<Partial<LotteryNews>>(request);
    } catch (error) {
      return jsonResponse({ error: error instanceof Error ? error.message : "Invalid news body" }, { status: 400 });
    }

    if (!newsItem.title || !newsItem.content || !newsItem.category || !newsItem.image) {
      return jsonResponse({ error: "Title, content, category, and image are required" }, { status: 400 });
    }

    const newItem: LotteryNews = {
      id: createId(),
    ...
    await writeStoreCollection("news", nextNews);

    // Broadcast to subscribers
    for (const subscriber of newsSubscribers) {
      try {
        subscriber({ type: "news_update", news: [newItem] });
      } catch (error) {
  ...
  if (url.pathname === "/api/news" && request.method === "DELETE") {
    ...
    try {
      payload = await readJsonBody<{ id?: string }>(request);
    } catch (error) {
      return jsonResponse({ error: error instanceof Error ? error.message : "Invalid JSON body" }, { status: 400 });
    }

    if (!payload.id) {
      return jsonResponse({ error: "News item ID is required" }, { status: 400 });
    }

    const success = await deleteFromStoreCollection<LotteryNews>("news", (item) => item.id === payload.id);
  ...
  if (url.pathname === "/api/news/stream" && request.method === "GET") {
    ...
    const sendFn = (data: unknown) => channel.send(data);
    newsSubscribers.add(sendFn);

    // Send initial news
    try {
      const news = await readStoreCollection<LotteryNews>("news");
      channel.send({ type: "initial_news", news });
    } catch (error) {
