...
function AdminPage() {
  ...
  const [vehicles, setLotteryPrizes] = useState<LotteryPrize[]>([]);
  ...
  const refreshData = async () => {
    ...
    try {
      setLotteryPrizes(await apiGet<LotteryPrize[]>("/api/vehicles"));
    } catch (error) {
      console.error(error);
      setUploadError("Could not load prizes from the database.");
    }
    try {
      setNews(await apiGet<LotteryNews[]>("/api/news"));
    } catch (error) {
      console.error(error);
    }
    try {
  ...
  const addLotteryPrize = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const form = event.currentTarget;
    if (cardImageFiles.length === 0 && galleryImageFiles.length === 0) {
      setUploadError("Add at least one prize image before saving.");
      return;
    }
    setUploadError("");
    setSavingLotteryPrize(true);

    try {
      const formData = new FormData(form);
      formData.delete("cardImages");
      formData.delete("galleryImages");
      formData.delete("detailsPackage");

      // Capture draw location
      const drawLocation = formData.get("drawLocation") as string | null;
      if (drawLocation) {
        formData.set("drawUrl", drawLocation);
        formData.delete("drawLocation");
      }

      for (const item of cardImageFiles) {
        const prepared = await prepareImageFile(item.file);
        formData.append("cardImages", prepared, prepared.name);
      }
      for (const item of galleryImageFiles) {
        const prepared = await prepareImageFile(item.file);
        formData.append("galleryImages", prepared, prepared.name);
      }
      if (detailsPackageFile) {
        formData.append("detailsPackage", detailsPackageFile, detailsPackageFile.name);
      }
      ...
      if (!response.ok) {
        ...
        throw new Error(payload?.error ?? `Lottery prize upload failed with ${response.status}`);
      }

      const vehicle = (await response.json()) as LotteryPrize;
      const nextLotteryPrizes = [vehicle, ...vehicles];
      setLotteryPrizes(nextLotteryPrizes);
      form?.reset();
      clearSelectedImages();
      ...
      setUploadError(
        isStorageQuotaError(error)
          ? "The prize was not saved because browser storage is full. Try fewer images, smaller images, or remove an old prize first."
          : error instanceof Error
            ? error.message
            : "The prize was not saved because the selected files could not be processed. Try different image files.",
      );
    } finally {
  ...
  const removeLotteryPrize = async (id: number) => {
    try {
      const response = await apiSend<{ ok: boolean; error?: string; remainingCount?: number }>(`/api/vehicles/${id}`, "DELETE");
      if (response.ok) {
        const nextLotteryPrizes = vehicles.filter((vehicle) => vehicle.id !== id);
        setLotteryPrizes(nextLotteryPrizes);
        setUploadError("");
      } else {
  ...
  const addNews = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const newsItem = {
      ...
      category: String(formData.get("category") || "update") as "draw" | "winner" | "announcement" | "update",
      image: String(formData.get("image") || ""),
      link: String(formData.get("link") || "") || undefined,
    };

    try {
      const response = await apiSend<LotteryNews>("/api/news", "POST", newsItem);
  ...
  const deleteNews = async (id: string) => {
    ...
    try {
      await apiSend<{ success: boolean }>("/api/news", "DELETE", { id });
    ...
    <main className="admin-page min-h-screen bg-[#f5f7fb] px-4 py-8 text-[#172033] sm:px-6 lg:px-8">
      <div className="mx-auto w-full max-w-[1760px]">
        ...
        <section className="mt-5 grid gap-5">
          <AdminPanel title="Add Lottery Prize">
            <form onSubmit={addLotteryPrize} encType="multipart/form-data" className="grid gap-4 grid-cols-1">
              <input name="name" placeholder="Prize Name" className="admin-field" />
              <select name="prizeTier" className="admin-field">
                <option value="">Select Prize Tier...</option>
                <option value="GRAND">GRAND</option>
                <option value="MAJOR">MAJOR</option>
                <option value="STANDARD">STANDARD</option>
                <option value="BASIC">BASIC</option>
              </select>
              <input name="prizeAmount" type="number" placeholder="Prize Amount ($)" className="admin-field" />
              <input name="odds" placeholder="Odds (e.g., 1 in 50,000,000)" className="admin-field" />
              <input name="ticketPrice" type="number" placeholder="Ticket Price ($)" className="admin-field" />
              <input name="totalTickets" type="number" placeholder="Total Tickets" className="admin-field" />
              <select name="drawLocation" className="admin-field">
                <option value="">Select Lottery Draw Location...</option>
                <option value="https://www.powerball.com">Powerball (USA)</option>
                <option value="https://www.megamillions.com">Mega Millions (USA)</option>
                <option value="https://www.calottery.com">California Lottery (USA)</option>
                <option value="https://nylottery.ny.gov">New York Lottery (USA)</option>
                <option value="https://www.txlottery.org">Texas Lottery (USA)</option>
                <option value="https://www.flalottery.com">Florida Lottery (USA)</option>
                <option value="https://www.illinoislottery.com">Illinois Lottery (USA)</option>
                <option value="https://www.ohiolottery.com">Ohio Lottery (USA)</option>
                <option value="https://www.michiganlottery.com">Michigan Lottery (USA)</option>
                <option value="https://www.palottery.com">Pennsylvania Lottery (USA)</option>
                <option value="https://www.national-lottery.co.uk">National Lottery (UK)</option>
                <option value="https://www.ozlotteries.com">Oz Lotteries (Australia)</option>
                <option value="https://www.loto-quebec.com">Loto-Québec (Canada)</option>
                <option value="https://www.bclc.com">BC Lottery (Canada)</option>
                <option value="https://www.olg.ca">Ontario Lottery (Canada)</option>
              </select>
              <motion.div
              ...
              <textarea
                ...
                required
                rows={4}
                placeholder="Detailed prize specifications: prize description, eligibility requirements, draw schedule, claim process..."
                className="admin-field resize-none"
              />
              <button
                type="submit"
                disabled={savingLotteryPrize}
                className="rounded-full bg-[#e8a838] py-3 font-bold text-[#0a1628] disabled:cursor-wait disabled:opacity-65"
              >
                {savingLotteryPrize ? "Uploading..." : "Add Lottery Prize"}
              </button>
            </form>
          </AdminPanel>

          <AdminPanel title="Current Lottery Prizes">
                      ...
                      <td className="rounded-l-xl px-3 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-white/45">{vehicle.prizeTier}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-3 py-3">
        ...
        <section className="mt-5 grid gap-5">
          <AdminPanel title="Add Lottery News">
            <form onSubmit={addNews} className="grid gap-4 grid-cols-1">
              <input name="title" placeholder="News Title" className="admin-field" />
              <textarea name="content" placeholder="News Content" className="admin-field" rows={3} />
              <select name="category" className="admin-field">
                <option value="draw">Draw Update</option>
                <option value="winner">Winner Announcement</option>
                <option value="announcement">Announcement</option>
                <option value="update">General Update</option>
              </select>
              <input name="image" placeholder="Image URL" className="admin-field" />
              <input name="link" placeholder="Link URL (optional)" className="admin-field" />
              <button
                type="submit"
                className="rounded-full bg-[#e8a838] px-5 py-2 font-bold text-[#0a1628] hover:bg-[#d9942f] transition-colors"
              >
                Add News
              </button>
            </form>
