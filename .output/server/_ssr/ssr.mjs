...
async function sendVerificationEmail({ email, code }) {
	try {
		...
		if (error instanceof Error) {
			if (error.message.includes("401") || error.message.includes("Unauthorized")) throw new Error("SendGrid authentication failed: Invalid API key. Please check your SENDGRID_API_KEY configuration.");
			if (error.message.includes("403") || error.message.includes("Forbidden")) throw new Error("SendGrid access denied: Your API key may not have permission to send emails. Check your SendGrid API key permissions.");
			if (error.message.includes("429") || error.message.includes("rate limit")) throw new Error("SendGrid rate limit exceeded: You've sent too many emails. Please wait and try again later.");
